<#
.SYNOPSIS
    Winget 사용자 정의 저장소 통합 패키지 추가 도구 (v5.1 Final)
.DESCRIPTION
    1. GitHub Repo URL 입력 및 중복 ID 체크 (Skip 로직 포함)
    2. packages.yml 무결성 보장 업데이트
    3. wingetcreate 분기 처리 (New vs Update)
    4. Git Pull(Rebase) -> Commit (Action 명시) -> Push
#>
...
do {
    Clear-Host
    Write-Log "=== Winget 패키지 추가 도구 (v5.1 Final) ===" -Color Cyan
    ...
    # --- [3] GitHub Release Info & Asset Selection ---
    Write-Log "🔍 $fullRepo 릴리스 정보 조회 중..." -Color Cyan
    try {
        $releaseInfo = gh api "repos/$fullRepo/releases/latest" | ConvertFrom-Json
    } catch {
        Write-Log "릴리스 정보를 가져올 수 없습니다. (권한 혹은 존재 여부 확인)" -Level ERROR
        continue
    }

    if (-not $releaseInfo.assets) {
        Write-Log "릴리스에 자산(Assets)이 없습니다." -Level ERROR
        continue
    }
...
if ($addedPackages.Count -gt 0) {
    ...
    # [수정됨] 리뷰어 제안 수용: Action별 커밋 메시지 생성
